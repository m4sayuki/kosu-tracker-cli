...
class TestIsMonitorCommand:
    def test_module_run_monitor_command_returns_true(self):
    ...
    def test_unrelated_command_containing_words_returns_false(self):
        command = "/usr/bin/vim kosu run-monitor notes.txt"
        assert is_monitor_command(command) is False

    def test_other_kosu_command_returns_false(self):
        command = "/venv/bin/kosu status"
        assert is_monitor_command(command) is False


class TestIsMonitorProcess:
    def test_module_run_monitor_process_returns_true(self, mocker):
    ...
    def test_console_script_run_monitor_process_returns_true(self, mocker):
        mocker.patch("kosu_tracker.cli.is_pid_running", return_value=True)
        mock_run = mocker.patch("kosu_tracker.cli.subprocess.run")
