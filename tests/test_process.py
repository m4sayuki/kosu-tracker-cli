...
class TestProcessCommand:
    def test_success_returns_stripped_command(self, mocker):
        mocker.patch(
            "kosu_tracker.cli.subprocess.run",
            return_value=MagicMock(returncode=0, stdout="python -m kosu_tracker.cli run-monitor\n"),
        )
...
class TestIsMonitorProcess:
    ...
    def test_false_for_reused_pid_with_other_command(self, mocker):
        mocker.patch("kosu_tracker.cli.is_pid_running", return_value=True)
        mocker.patch("kosu_tracker.cli.process_command", return_value="sleep 100")
        assert is_monitor_process(123) is False
...
class TestStopMonitor:
    ...
    def test_active_monitor_is_signaled(self, mocker):
        cli_module.PID_FILE.write_text("12345", encoding="utf-8")
...
class TestPrintStatus:
    def test_reused_pid_reports_not_running(self, mocker, capsys):
        cli_module.PID_FILE.write_text("12345", encoding="utf-8")
        mocker.patch("kosu_tracker.cli.is_monitor_process", return_value=False)
