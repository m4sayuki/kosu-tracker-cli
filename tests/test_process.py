...
class TestIsMonitorProcess:
    ...
    def test_console_script_run_monitor_process_returns_true(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="/tmp/.venv/bin/kosu run-monitor --interval 60\n",
        )
    ...
    def test_unrelated_running_process_returns_false(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="/usr/bin/python3 unrelated.py\n",
        )

        assert is_monitor_process(12345) is False

    def test_non_running_pid_returns_false_without_ps(self, mocker):
...
class TestRequireNotRunning:
    ...
    def test_running_unrelated_pid_file_is_deleted(self, mocker):
