...
from kosu_tracker.cli import is_monitor_command, is_pid_running, read_pid, require_not_running, stop_monitor
...
class TestReadPid:
    ...
    def test_zero_pid_file_returns_none(self):
        cli_module.PID_FILE.write_text("0", encoding="utf-8")
        assert read_pid() is None

    def test_negative_pid_file_returns_none(self):
...
class TestIsMonitorCommand:
    def test_python_module_run_monitor_is_monitor(self):
        assert is_monitor_command("/usr/bin/python -m kosu_tracker.cli run-monitor --interval 60")

    def test_console_script_run_monitor_is_monitor(self):
        assert is_monitor_command("/usr/local/bin/kosu run-monitor --interval 60")

    def test_python_module_without_run_monitor_is_not_monitor(self):
        assert not is_monitor_command("/usr/bin/python -m kosu_tracker.cli status")

    def test_unrelated_command_with_run_monitor_arg_is_not_monitor(self):
        assert not is_monitor_command("/bin/sh -c 'echo run-monitor'")
...
class TestRequireNotRunning:
    ...
    def test_unrelated_live_pid_is_deleted_as_stale(self):
        cli_module.PID_FILE.write_text(str(os.getpid()), encoding="utf-8")
        require_not_running()
        assert not cli_module.PID_FILE.exists()
