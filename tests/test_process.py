...
class TestIsMonitorCommand:
    ...
    def test_monitor_arguments_on_unrelated_process_do_not_match(self):
        assert not _is_monitor_command("/usr/bin/python3 script.py kosu run-monitor")

    def test_shell_command_does_not_match(self):
        assert not _is_monitor_command("/bin/sh -c 'kosu run-monitor'")

    def test_unparseable_command_does_not_match(self):
        assert not _is_monitor_command("python3 -m 'unterminated")
...
class TestRequireNotRunning:
    ...
    def test_unrelated_active_pid_file_is_deleted(self, mocker):
...
class TestStopMonitor:
    def test_refuses_to_signal_unrelated_live_pid(self, mocker):
        pid = os.getpid()
        cli_module.PID_FILE.write_text(str(pid), encoding="utf-8")
        mocker.patch("kosu_tracker.cli.is_monitor_process", return_value=False)
    ...
    def test_pid_file_is_preserved_when_monitor_does_not_stop(self, mocker):
        cli_module.PID_FILE.write_text("123", encoding="utf-8")
        mocker.patch("kosu_tracker.cli.is_monitor_process", return_value=True)
...
class TestIntervalValidation:
    ...
    def test_start_monitor_rejects_negative_interval(self, mocker):
