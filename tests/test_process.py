"""Tests for read_pid, process checks, require_not_running, and stop_monitor."""
...
class TestIsMonitorProcess:
    def test_module_run_monitor_command_is_monitor(self):
        assert is_monitor_command("/usr/bin/python -m kosu_tracker.cli run-monitor --interval 60") is True

    def test_console_script_run_monitor_command_is_monitor(self):
        assert is_monitor_command("/venv/bin/kosu run-monitor --interval 60") is True

    def test_unrelated_command_with_similar_words_is_not_monitor(self):
        assert is_monitor_command("/tmp/kosu-helper run-monitor --interval 60") is False

    def test_not_running_pid_returns_false(self, mocker):
        mocker.patch("kosu_tracker.cli.is_pid_running", return_value=False)
        mock_process_command = mocker.patch("kosu_tracker.cli.process_command")

        assert is_monitor_process(12345) is False
        mock_process_command.assert_not_called()
    ...
    def test_reused_pid_for_unrelated_process_returns_false(self, mocker):
        mocker.patch("kosu_tracker.cli.is_pid_running", return_value=True)
        mocker.patch(
            "kosu_tracker.cli.process_command",
            return_value="/Applications/TextEdit.app/Contents/MacOS/TextEdit",
        )
...
class TestRequireNotRunning:
    ...
    def test_reused_pid_file_is_deleted(self, mocker):
