...
class TestBuildParser:
    ...
    def test_start_rejects_negative_interval(self):
        with pytest.raises(SystemExit):
    ...
    def test_report_rejects_zero_interval_minutes(self):
        with pytest.raises(SystemExit):
            self._parse(["report", "--interval-minutes", "0"])
    ...
    def test_run_monitor_rejects_negative_interval(self):
        with pytest.raises(SystemExit):
            self._parse(["run-monitor", "--interval", "-10"])
